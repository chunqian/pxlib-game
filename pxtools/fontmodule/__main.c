...
int PX_Printf(const char *format, ...) {
    px_char text[256] = {0};
    px_char dstText[256] = {0};
    va_list args;
    va_start(args, format);
    vsprintf(text, format, args);  // 使用 sprintf 有问题
    utf8_to_gb(text, dstText, 256);
    va_end(args);
    return printf(dstText);
}
...
int Font_Build(px_char *font_file, px_char *asc_file, px_char *_out, px_int fontsize, PX_FONTMODULE_CODEPAGE codePage) {
    ...
    while (offset < size) {
        ...
        PX_memcpy(character, &code, 4);

        if (!(codesize = Font_BuildFontBuffer(code, utf16_code, codePage))) {
            PX_Printf("生成字模:\"%s\" 错误\n", character);
            goto _ERROR;
        }
        if (fwrite(cache_buffer, 1, codesize, pf) != codesize) goto _ERROR;
...
int main(int argc, const px_char *argv[]) {
    ...
    PX_Printf("编码类型(0-gbk/1-utf8/2-utf16):");
    scanf("%d", &codepage);
