...
DWORD WINAPI DEMO_DSoundProc(LPVOID p) {
    px_dword wsize = 0;
...
BOOL PX_AudioInitialize(PX_SoundPlay *soundPlay, BOOL startThread) {
    ...
    if (DSound_lpdbsBuffer->Lock(0, DSOUND_BUFFER_SIZE, &pDSLockedBuffer, &dwDSLockedBufferSize, NULL, NULL, 0)) return FALSE;
    ...
    if (startThread) hThread = CreateThread(NULL, 0, DEMO_DSoundProc, 0, 0, &threadId);
...
int PX_AudioWriteBuffer(void *pBuffer, size_t Size) {
    ...
    if (DSound_lpdbsBuffer->Lock(DSound_wPosition, wbuf1_size, &pDSLockedBuffer, &dwDSLockedBufferSize, NULL, NULL, 0L)) return FALSE;
    ...
    if (Size) {
        ...
        if (DSound_lpdbsBuffer->Lock(0, wbuf2_size, &pDSLockedBuffer, &dwDSLockedBufferSize, NULL, NULL, 0L)) return FALSE;
...
BOOL PX_AudioCaptureOpen(GUID dev_id, int channel) {
    ...
    if (FAILED(hr = DSoundCapture_pCapDev->CreateCaptureBuffer(&dscbd, &DSoundCapture_pCapBuf, NULL))) return PX_FALSE;
...
DWORD PX_AudioCaptureReadEx(void *pBuffer, px_int buffersize, px_int align) {
    ...
    if (DSoundCapture_pCapBuf->Lock(DSoundCapture_RPosition, rbuf1_size, &pDSLockedBuffer, &dwDSLockedBufferSize, NULL, NULL, 0L)) return FALSE;
    ...
    if (ReadySize) {
        ...
        if (DSoundCapture_pCapBuf->Lock(0, rbuf2_size, &pDSLockedBuffer, &dwDSLockedBufferSize, NULL, NULL, 0L)) return FALSE;
...
DWORD PX_AudioCaptureRead(void *pBuffer, px_int buffersize) { return PX_AudioCaptureReadEx(pBuffer, buffersize, 1); }
