...
px_void PX_MessageBoxClose(PX_MessageBox *pm) { pm->mode = PX_MESSAGEBOX_MODE_CLOSE; }
...
px_void PX_MessageBoxRender(px_surface *pSurface, PX_MessageBox *pm, px_dword elpased) {
    ...
    if (pm->schedule < PX_MESSAGEBOX_DEFAULT_STAGE_1_TIME) {
        ...
        PX_GeoDrawRect(pSurface, (pm->window_Width - width) / 2, (pm->window_Height - pm->PX_MESSAGEBOX_STAGE_1_HEIGHT) / 2, (pm->window_Width + width) / 2,
                       (pm->window_Height + pm->PX_MESSAGEBOX_STAGE_1_HEIGHT) / 2, backGroundColor);
    } else if (pm->schedule < PX_MESSAGEBOX_DEFAULT_STAGE_2_TIME) {
        px_int height =
            PX_MESSAGEBOX_DEFAULT_STAGE_2_HEIGHT * pm->schedule * pm->schedule / (PX_MESSAGEBOX_DEFAULT_STAGE_2_TIME * PX_MESSAGEBOX_DEFAULT_STAGE_2_TIME);
        PX_GeoDrawRect(pSurface, 0, (pm->window_Height - height) / 2, pm->window_Width - 1, (pm->window_Height + height) / 2, backGroundColor);
    } else {
        PX_GeoDrawRect(pSurface, 0, (pm->window_Height - pm->PX_MESSAGEBOX_STAGE_2_HEIGHT) / 2, pm->window_Width - 1,
                       (pm->window_Height + pm->PX_MESSAGEBOX_STAGE_2_HEIGHT) / 2, backGroundColor);
        PX_GeoDrawRect(pSurface, 0, (pm->window_Height - pm->PX_MESSAGEBOX_STAGE_2_HEIGHT) / 2 - 30, pm->window_Width - 1,
                       (pm->window_Height - pm->PX_MESSAGEBOX_STAGE_2_HEIGHT) / 2 - 10, backGroundColor);
        PX_GeoDrawRect(pSurface, 0, (pm->window_Height + pm->PX_MESSAGEBOX_STAGE_2_HEIGHT) / 2 + 10, pm->window_Width - 1,
...
px_void PX_MessageBoxAlertYesNoEx(PX_MessageBox *pm, const char *Message, PX_MessageBoxCallBack func_yescallback, px_void *yesptr,
                                  PX_MessageBoxCallBack func_nocallback, px_void *noptr) {
...
px_void PX_MessageBoxSetFontModule(PX_MessageBox *pm, PX_FontModule *fm) { pm->fontmodule = fm; }
