...
px_bool PX_ParticalLauncherCreate(PX_Partical_Launcher *env, px_memorypool *mp, px_texture *tex, PX_ScriptVM_Instance *pIns, px_char *Initfunc,
                                  px_char *_createfunc, px_char *_updatefunc) {
    px_int vm_ptr;
...
px_void PX_ParticalAtomUpdate(PX_Partical_Launcher *env, PX_Partical_Atom *pAtom, px_dword elpased) {
    ...
    */
        if (env->Update_func) {
            ...
            if (!PX_ScriptVM_LocalAlloc(env->VM_Instance, 18, &memptr)) return;
...
px_void PX_ParticalLauncherRender(px_surface *surface, PX_Partical_Launcher *env, px_point offset) {
    ...
    if (!PX_isRectCrossRect(
            PX_RECT(0, 0, (px_float)surface->width, (px_float)surface->height),
    ...
    for (i = 0; i < (px_int)env->maxCount; i++) {
        ...
        if (env->ParticalPool[i].aliveTime != 0 && env->ParticalPool[i].size != 0) {
            if (env->ParticalPool[i].size != 1.0 || env->ParticalPool[i].rotation != 0) {
                ...
                if (env->ParticalPool[i].alpha == 1.0 && env->ParticalPool[i].hdrR == 1.0f && env->ParticalPool[i].hdrG == 1.0f &&
                    env->ParticalPool[i].hdrB == 1.0f)
                    PX_TextureRenderEx(surface, env->texture, (px_int)(pos.x), (px_int)(pos.y), PX_TEXTURERENDER_REFPOINT_CENTER, PX_NULL,
                                       env->ParticalPool[i].size, env->ParticalPool[i].rotation);
                else {
                    ...
                    PX_TextureRenderEx(surface, env->texture, (px_int)(pos.x), (px_int)(pos.y), PX_TEXTURERENDER_REFPOINT_CENTER, &blend,
                                       env->ParticalPool[i].size, env->ParticalPool[i].rotation);
                }
            } else {
                if (env->ParticalPool[i].alpha == 1.0 && env->ParticalPool[i].hdrR == 1.0f && env->ParticalPool[i].hdrG == 1.0f &&
...
px_void PX_ParticalLauncherFree(PX_Partical_Launcher *env) { MP_Free(env->mp, env->ParticalPool); }
...
px_void PX_ParticalLauncherSetDirection(PX_Partical_Launcher *launcher, px_point direction) { launcher->direction = direction; }
