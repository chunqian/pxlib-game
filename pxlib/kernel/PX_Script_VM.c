...
static px_void PX_ScriptVM_Error(PX_ScriptVM_Instance *Ins, px_char *log) { PX_ERROR(log); }
...
PX_SCRIPTVM_RUNRETURN PX_ScriptVM_InstanceRunThread(PX_ScriptVM_Instance *Ins, px_int tick) {
    ...
    while (PX_TRUE) {
        ...
        if (opCode != PX_SCRIPT_ASM_INSTR_OPCODE_MOV && opCode != PX_SCRIPT_ASM_INSTR_OPCODE_ADD && opCode != PX_SCRIPT_ASM_INSTR_OPCODE_SUB &&
            opCode != PX_SCRIPT_ASM_INSTR_OPCODE_PUSH && opCode != PX_SCRIPT_ASM_INSTR_OPCODE_POP) {
        ...
        switch (opCode) {
            ...
            case PX_SCRIPT_ASM_INSTR_OPCODE_STRCHR: {
                ...
                if (sVar._int < PX_strlen(cVar._string.buffer)) pVar->_int = (px_uchar)cVar._string.buffer[sVar._int];
                pT->IP += (4 + 3 * 4);
            } break;
            ...
            case PX_SCRIPT_ASM_INSTR_OPCODE_MEMSET: {
                ...
                if (sVar._int > pVar->_memory.usedsize - 1) pVar->_memory.usedsize = sVar._int + 1;
            ...
            case PX_SCRIPT_ASM_INSTR_OPCODE_MEMTSTR: {
                ...
                if (pVar->type == PX_SCRIPTVM_VARIABLE_TYPE_MEMORY) {
                    if (pVar->_memory.usedsize == 0) {
                        ...
                        if (PX_strlen((px_char *)pVar->_memory.buffer) == pVar->_memory.usedsize - 1) PX_StringCatChar(&cVar._string, lastchar);
...
px_bool PX_ScriptVM_InstanceRunFunction(PX_ScriptVM_Instance *Ins, px_int threadID, px_void *runParam, px_char *func, PX_SCRIPTVM_VARIABLE args[],
                                        px_int paramcount) {
    int i, j, ip;
...
px_bool PX_ScriptVM_InstanceRunFunctionIndex(PX_ScriptVM_Instance *Ins, px_int threadID, px_void *runParam, px_int funcIndex, PX_SCRIPTVM_VARIABLE args[],
                                             px_int paramcount) {
    int i, j, ip;
    px_int old_T;
    if (paramcount > 16 || threadID >= Ins->maxThreadCount) {
...
px_bool PX_ScriptVM_InstanceBeginThreadFunction(PX_ScriptVM_Instance *Ins, px_int threadID, px_void *runParam, px_char *func, PX_SCRIPTVM_VARIABLE args[],
                                                px_int paramcount) {
    int i, j, ip;
    px_int old_T;
    px_char uprname[__PX_SCRIPT_ASM_MNEMONIC_NAME_LEN];
...
px_bool PX_ScriptVM_InstanceBeginThreadFunctionIndex(PX_ScriptVM_Instance *Ins, px_int threadID, px_void *runParam, px_int funcIndex,
                                                     PX_SCRIPTVM_VARIABLE args[], px_int paramcount) {
    int i, j, ip;
    px_int old_T;
    if (paramcount > 16 || threadID >= Ins->maxThreadCount) {
        return PX_FALSE;
...
px_bool PX_ScriptVM_InstanceFree(PX_ScriptVM_Instance *Ins) {
    ...
    if (Ins->_mem) MP_Free(Ins->mp, Ins->_mem);

    if (Ins->_func) MP_Free(Ins->mp, Ins->_func);

    if (Ins->_host) MP_Free(Ins->mp, Ins->_host);

    if (Ins->_string) MP_Free(Ins->mp, Ins->_string);

    if (Ins->_bin) MP_Free(Ins->mp, Ins->_bin);

    if (Ins->_memory) MP_Free(Ins->mp, Ins->_memory);
    ...
    while (PX_TRUE) {
        ...
        if (PX_strequ(lexer.CurLexeme.buffer, "B")) {
            ...
            for (i = 0; i < u; i++) {
                ...
                oft += instrLen;
                if (instrLen) g_scriptVM_printFunc("\n");
            }
            break;
...
px_void PX_ScriptVM_DebuggerInterrupt(PX_ScriptVM_Instance *Ins) {
    ...
    if ((Ins->bp_IP == pT->IP) || Ins->bp_next) {
        if (Ins->bp_next) Ins->bp_IP = PX_FALSE;
    } else {
        return;
    }
