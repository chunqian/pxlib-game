...
px_bool PX_SyncFrameServerInit(PX_SyncFrame_Server *sync, px_memorypool *mp, px_dword updateDuration, px_syncframe_server_read read,
                               px_syncframe_server_write write, px_void *user) {
    sync->mp = mp;
    sync->read = read;
    sync->write = write;
    sync->time = 0;
...
px_void PX_SyncFrameServerSetVersion(PX_SyncFrame_Server *sync, px_dword version) { sync->version = version; }
            ...
            for (i = 0; i < sync_server->clients.size; i++) {
                ...
                if (pClient->sendDurationTick >= PX_SYNC_SERVER_SEND_DURATION) {
                    ...
                    if (pClient->sendTimes <= 0) {
                        ...
                        PX_memcpy(pClient->send_cache_instr_buffer + (sizeof(sync_server->recv_cache_buffer) - reserve_size), &pClient->timeStreamOffset,
                                  sizeof(pClient->timeStreamOffset));
                        reserve_size -= sizeof(pClient->timeStreamOffset);
                        // stream size 4bytes
                        PX_memcpy(pClient->send_cache_instr_buffer + (sizeof(sync_server->recv_cache_buffer) - reserve_size), &wstream_size,
                                  sizeof(wstream_size));
                        reserve_size -= sizeof(wstream_size);

                        // Instr stream
                        if (wstream_size > reserve_size) {
                            wstream_size = reserve_size;
                        }
                        PX_memcpy(pClient->send_cache_instr_buffer + (sizeof(sync_server->recv_cache_buffer) - reserve_size),
                                  sync_server->stampsInstrStream.buffer + pClient->timeStreamOffset, wstream_size);
                        ...
                        if (reserve_size >= sizeof(PX_SyncFrame_InstrStream_StampIndex) + sizeof(px_dword) * 2) {
                            if (sync_server->stampsIndexTable.size - pClient->timeIndexOffset > 1)  // last stamp is not ready
                            {
                                ...
                                if (windexTableSize) {
                                    // Index start-offset
                                    PX_memcpy(pClient->send_cache_instr_buffer + (sizeof(sync_server->recv_cache_buffer) - reserve_size),
                                              &pClient->timeIndexOffset, sizeof(pClient->timeIndexOffset));
                                    reserve_size -= sizeof(pClient->timeIndexOffset);
                                    // Index Size
                                    PX_memcpy(pClient->send_cache_instr_buffer + (sizeof(sync_server->recv_cache_buffer) - reserve_size), &windexTableSize,
                                              sizeof(windexTableSize));
                                    reserve_size -= sizeof(windexTableSize);
                                    // data
                                    PX_memcpy(
                                        pClient->send_cache_instr_buffer + (sizeof(sync_server->recv_cache_buffer) - reserve_size),
                                        (px_byte *)sync_server->stampsIndexTable.data + pClient->timeIndexOffset * sizeof(PX_SyncFrame_InstrStream_StampIndex),
...
px_bool PX_SyncFrameServerAddClient(PX_SyncFrame_Server *sync, px_dword server_verify_id, px_dword client_id, px_dword c_id) {
    ...
    for (i = 0; i < PX_SYNC_UNIQUE_ARRAY_SIZE; i++) client.acceptuniqueQueue[i] = 0;
...
px_void PX_SyncFrameServerSetInstrOnceMaxSize(PX_SyncFrame_Server *sync, px_int maxsize) { sync->instr_once_maxsize = maxsize; }

px_bool PX_SyncFrameClientInit(PX_SyncFrame_Client *client, px_memorypool *mp, px_dword updateDuration, PX_Sync_Port serverport, px_dword server_verify_id,
                               px_dword client_id, px_syncframe_client_read read, px_syncframe_client_write write, px_void *user) {
    px_int i;
...
px_int PX_SyncFrameClientGetReadyFrameCount(PX_SyncFrame_Client *sync) { return sync->stampsIndexTable.size; }
...
px_void PX_SyncFrameClientSetVersion(PX_SyncFrame_Client *sync, px_dword version) { sync->version = version; }

px_void PX_SyncFrameServerStop(PX_SyncFrame_Server *sync) { sync->status = PX_SYNC_SERVER_STATUS_END; }
px_void PX_SyncFrameServerRun(PX_SyncFrame_Server *sync) { sync->status = PX_SYNC_SERVER_STATUS_CONNECT; }
...
px_int PX_SyncFrameServerGetReadyFrameCount(PX_SyncFrame_Server *sync) { return sync->stampsIndexTable.size - 1; }
...
px_bool PX_SyncDataServerInit(PX_SyncData_Server *syncdata_server, px_memorypool *mp, px_dword serverID, px_syncdata_server_read read,
                              px_syncdata_server_write write, px_void *user) {
...
px_void PX_SyncDataServerFree(PX_SyncData_Server *syncdata_server) { PX_VectorFree(&syncdata_server->clients); }

px_bool PX_SyncDataClientInit(PX_SyncData_Client *syncdata_client, px_memorypool *mp, px_dword serverID, px_dword clientID, PX_Sync_Port server_port,
                              px_syncdata_client_read read, px_syncdata_client_write write, px_void *user) {
...
px_bool PX_SyncDataClientIsCompleted(PX_SyncData_Client *syncdata_client) { return (syncdata_client->status == PX_SYNC_SERVERCLIENT_STATUS_END); }
