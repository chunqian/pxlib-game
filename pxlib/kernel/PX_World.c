...
px_void PX_WorldEnableAuxiliaryLine(PX_World *pw, px_bool bline) { pw->auxiliaryline = bline; }
...
px_void PX_WorldUpdate(PX_World *world, px_uint elpased) {
    ...
    for (i = 0; i < updateCount; i++) {
        ...
        if (pwo->pObject->impact_Object_type) {
            j = 0;
            while (!(pwo->pObject->impact_Object_type & (1 << j))) j++;
        ...
        if (pwo->pObject->impact_Object_type) {
            for (b = 0; b < sizeof(pwo->pObject->impact_Object_type) * 8; b++) {
                if ((pwo->pObject->impact_Object_type & (1 << b))) {
                    ...
                    userData.ptr = pwo;
                    PX_QuadtreeAddNode(&world->Impact_Test_array[b], (px_float)pwo->pObject->x, (px_float)pwo->pObject->y, (px_float)pwo->pObject->Width,
                                       (px_float)pwo->pObject->Height, userData);
                }
            }
        }
    }
        ...
        if (pwo->pObject->impact_test_type) {
            for (b = 0; b < sizeof(pwo->pObject->impact_test_type) * 8; b++) {
                if ((pwo->pObject->impact_test_type & (1 << b))) {
                    ...
                    PX_QuadtreeTestNode(&world->Impact_Test_array[b], (px_float)pwo->pObject->x, (px_float)pwo->pObject->y, (px_float)pwo->pObject->Width,
                                        (px_float)pwo->pObject->Height, userData);
                    for (im_i = 0; im_i < world->Impact_Test_array[b].Impacts.size; im_i++) {
                        ...
                        if (pObj1->diameter && pObj2->diameter) {
                            if (!PX_isCircleCrossCircle(PX_POINT(pObj1->x, pObj1->y, 0), pObj1->diameter / 2, PX_POINT(pObj2->x, pObj2->y, 0),
                                                        pObj2->diameter / 2))
                                continue;
                        } else if (pObj1->diameter == 0 && pObj2->diameter) {
                            if (!PX_isRectCrossCircle(PX_RECT(pObj1->x - pObj1->Width / 2, pObj1->y - pObj1->Height / 2, pObj1->Width, pObj1->Height),
                                                      PX_POINT(pObj2->x, pObj2->y, 0), pObj2->diameter / 2)) {
                                continue;
                            }
                        } else if (pObj1->diameter && pObj2->diameter == 0) {
                            if (!PX_isRectCrossCircle(PX_RECT(pObj2->x - pObj2->Width / 2, pObj2->y - pObj2->Height / 2, pObj2->Width, pObj2->Height),
                                                      PX_POINT(pObj1->x, pObj1->y, 0), pObj1->diameter / 2)) {
                                continue;
                            }
                        }
...
px_bool PX_WorldAddObject(PX_World *World, PX_Object *pObject) { return PX_VectorPushback(&World->pNewObjects, &pObject); }

px_void PX_WorldSetAuxiliaryLineColor(PX_World *pw, px_color color) { pw->auxiliaryline_color = color; }

px_point PX_WolrdObjectXYtoScreenXY(PX_World *pw, px_float x, px_float y) { return PX_POINT(x - pw->offsetx, y - pw->offsety, 0); }
...
_LIMIT px_int PX_WorldSearchRegion(PX_World *world, px_float x, px_float y, px_float raduis, PX_Object *Object[], px_int MaxSearchCount,
    ...
    for (b = 0; b < sizeof(impact_test_type) * 8; b++) {
        if ((impact_test_type & (1 << b))) {
            ...
            for (im_i = 0; im_i < world->Impact_Test_array[b].Impacts.size; im_i++) {
                ...
                if (pObj2->diameter) {
                    if (!PX_isCircleCrossCircle(PX_POINT(x, y, 0), raduis, PX_POINT(pObj2->x, pObj2->y, 0), pObj2->diameter / 2)) continue;
                } else {
                    if (!PX_isRectCrossCircle(PX_RECT(pObj2->x - pObj2->Width / 2, pObj2->y - pObj2->Height / 2, pObj2->Width, pObj2->Height),
                                              PX_POINT(x, y, 0), raduis)) {
                        continue;
                    }
                }
