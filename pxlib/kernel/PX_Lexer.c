...
px_char *PX_LexerIsCommentStart(px_lexer *lexer, const px_char ch[]) {
    ...
    for (i = 0; i < lexer->CommentCount; i++) {
        if (PX_memequ(ch, lexer->Comment[i].CommentBegin, PX_strlen(lexer->Comment[i].CommentBegin))) return lexer->Comment[i].CommentBegin;
    }
    return PX_NULL;
}
...
px_void PX_LexerRegisterContainerTransfer(px_lexer *lexer, px_uint containerIndex, px_char transfer) {
    if (containerIndex < PX_LEXER_CA_CONTAINER_MAX_COUNT) lexer->Container[containerIndex].transfer = transfer;
}

px_uint PX_LexerGetContainerType(px_lexer *lexer, px_char *pContainerText) { return lexer->CurrentContainerType; }

px_char *PX_LexerIsContainerStart(px_lexer *lexer, const px_char chr[]) {
    px_int i = 0;
    for (i = 0; i < lexer->ContainerCount; i++) {
        if (PX_memequ(chr, lexer->Container[i].ContainerBegin, PX_strlen(lexer->Container[i].ContainerBegin))) return lexer->Container[i].ContainerBegin;
    }
    return PX_NULL;
}
...
px_bool PX_LexerSortText(px_lexer *lexer, const px_char *SourceText) {
    ...
    while (*SourceText) {
        ...
        if (PX_LexerFilterChar(lexer, *SourceText)) {
            ...
            {
                if (PX_LexerIsNewLine(*SourceText) && PX_LexerIsSpacer(lexer, lexer->Sources[Offset - 1])) Offset--;
                if (PX_LexerIsDelimiter(lexer, *SourceText) && PX_LexerIsSpacer(lexer, lexer->Sources[Offset - 1])) Offset--;
            }
            lexer->Sources[Offset++] = *(SourceText);
        }
        SourceText++;
    ...
    while (*pNewSourceText) {
        // replace \r
        if (*pNewSourceText == '\r') *pNewSourceText = '\n';
        pNewSourceText++;
    }
...
px_int PX_LexerIsSourcsEnd(px_lexer *lexer) { return !lexer->Sources[lexer->SourceOffset]; }
...
PX_LEXER_LEXEME_TYPE PX_LexerGetNextLexeme(px_lexer *lexer) {
    ...
    PX_StringClear(&lexer->CurLexeme);
    lexer->Symbol = '\0';
    while (!PX_LexerIsSourcsEnd(lexer) && !PX_LexerIsDelimiter(lexer, lexer->Sources[lexer->SourceOffset]) &&
           !PX_LexerIsSpacer(lexer, lexer->Sources[lexer->SourceOffset]) && !PX_LexerIsNewLine(lexer->Sources[lexer->SourceOffset]) &&
...
px_void PX_LexerGetIncludedString(px_lexer *lexer, px_string *str) {
    px_int left, right;
    if (str != &lexer->CurLexeme) PX_StringCopy(str, &lexer->CurLexeme);
...
PX_LEXER_LEXEME_TYPE PX_LexerGetCurrentLexeme(px_lexer *lexer) { return lexer->CurrentLexemeFlag; }
...
px_int PX_LexerLoadSourceFromMemory(px_lexer *lexer, const px_char *buffer) { return PX_LexerSortText(lexer, buffer); }

px_void PX_LexerFree(px_lexer *lexer) {
    if (lexer->Sources) MP_Free(lexer->mp, lexer->Sources);

    PX_StringFree(&lexer->CurLexeme);
    lexer->Sources = PX_NULL;
}

px_void PX_LexerGetLexemeString(px_lexer *lexer, px_string *str) { PX_StringCopy(str, &lexer->CurLexeme); }
...
px_void PX_LexerSetState(PX_LEXER_STATE state) { state.lexer->SourceOffset = state.offset; }
...
px_bool PX_LexerIsLememeIsNumeric(px_lexer *lexer) { return PX_StringIsNumeric(&lexer->CurLexeme); }

px_void PX_LexerSetTokenCase(px_lexer *lexer, PX_LEXER_LEXEME_CASE _case) { lexer->lexemeTokenCase = _case; }
...
px_uint PX_LexerGetCurrentContainerType(px_lexer *lexer) { return lexer->CurrentContainerType; }

px_uint PX_LexerGetCurrentDelimiterType(px_lexer *lexer) { return lexer->CurrentDelimiterType; }

px_void PX_LexerSetNumericMatch(px_lexer *lexer, px_bool b) { lexer->NumericMath = b; }
