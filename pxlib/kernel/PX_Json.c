...
static px_bool PX_JsonInterpret_Value_Array(PX_Json *pjson, px_lexer *lexer, PX_Json_Value *_value) {
    ...
    if (type != PX_LEXER_LEXEME_TYPE_DELIMITER || lexer->Symbol != '[') goto _ERROR;
    ...
    while (PX_TRUE) {
        ...
        if (type == PX_LEXER_LEXEME_TYPE_DELIMITER) {
            ...
            if (!PX_ListPush(&_value->_Array, &array_value, sizeof(array_value))) goto _ERROR;
        ...
        if (type == PX_LEXER_LEXEME_TYPE_DELIMITER && lexer->Symbol == '{')  //{
        {
            ...
            if (type == PX_LEXER_LEXEME_TYPE_DELIMITER && lexer->Symbol == '}')  //{}
            {
                ...
                while (PX_TRUE) {
                    ...
                    if (!PX_ListPush(&json_Object->values, &_value, sizeof(_value))) goto _ERROR;
...
px_bool PX_JsonParse(PX_Json *pjson, const px_char *json_content) {
    ...
    if (!PX_JsonInterpret_Object(pjson, &lexer, &pjson->rootValue.json_Object)) goto _ERROR;
    PX_LexerFree(&lexer);
    return PX_TRUE;
_ERROR:
    PX_LexerFree(&lexer);
    ...
    switch (pValue->type) {
        ...
        case PX_JSON_VALUE_TYPE_OBJECT: {
            ...
            for (i = 0; i < pValue->json_Object.values.size; i++) {
                ...
                if (i != pValue->json_Object.values.size - 1)
                    if (!PX_StringCat(_out, ",")) return PX_FALSE;
                if (!PX_StringCat(_out, "\n")) return PX_FALSE;
            }
            if (!PX_StringCat(_out, "}")) return PX_FALSE;
...
px_bool PX_JsonBuild(PX_Json *pjson, px_string *_out) { return PX_JsonBuild_Value(&pjson->rootValue, _out, PX_TRUE); }
...
px_void PX_JsonFree(PX_Json *pjson) { PX_JsonFreeValue(pjson, &pjson->rootValue); }
