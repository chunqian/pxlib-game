...
px_bool PX_ResourceLibraryLoad(PX_ResourceLibrary *lib, PX_RESOURCE_TYPE type, px_byte *data, px_uint datasize, const px_char key[]) {
    ...
    switch (type) {
        ...
        case PX_RESOURCE_TYPE_TEXTURE:
            if (!PX_TextureCreateFromMemory(lib->mp, data, datasize, &res.texture)) return PX_FALSE;
            break;
...
PX_Resource *PX_ResourceLibraryGet(PX_ResourceLibrary *lib, const px_char key[]) { return (PX_Resource *)PX_MapGet(&lib->map, key); }
