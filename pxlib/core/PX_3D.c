...
px_bool PX_3D_CameraEulerInitialize(px_memorypool *mp, PX_3D_Camera *camera, px_point4D cameraPosition, px_float rotX, px_float rotY, px_float rotZ,
                                    px_float near, px_float far, px_float fov, px_float viewPortWidth, px_float viewPortHeight) {
...
px_bool PX_3D_CameraUVNInitialize(px_memorypool *mp, PX_3D_Camera *camera, px_point4D cameraPosition, px_point4D cameraTarget, px_float near, px_float far,
...
px_void PX_3D_RenderListPixelShader(px_surface *psurface, px_int x, px_int y, px_float z, px_float u, px_float v, px_point4D normal, px_texture *pTexture,
...
static px_void PX_3D_RenderListRasterization(px_surface *psurface, PX_3D_RenderList *pList, PX_3D_Vertex p0, PX_3D_Vertex p1, PX_3D_Vertex p2,
                                             px_texture *ptexture, px_color color, px_float zbuffer[], px_int zw) {
    px_int ix, iy;
    ...
    for (y = y0; y < midy; y++) {
        ...
        for (x = xleft, oneoverz = oneoverz_left, soverz = soverz_left, toverz = toverz_left; x < xright;
             ++x, oneoverz += oneoverz_step, soverz += soverz_step, toverz += toverz_step) {
            s = soverz / oneoverz;
            t = toverz / oneoverz;
...
px_void PX_3D_Present(px_surface *psurface, PX_3D_RenderList *list, PX_3D_Camera *camera) {
    ...
    if (ptex || clr._argb.ucolor) {
        for (i = 0; i < list->facestream.size; i++) {
            ...
            if (!(pface->state & PX_3D_FACESTATE_BACKFACE || pface->state & PX_3D_FACESTATE_CLIPPED)) {
                if (list->PX_3D_PRESENTMODE & PX_3D_PRESENTMODE_PURE) {
                    PX_3D_RenderListRasterization(psurface, list, pface->transform_vertex[0], pface->transform_vertex[1], pface->transform_vertex[2],
                                                  list->ptexture, clr, camera->zbuffer, (px_int)camera->viewport_width);
                } else {
                    PX_3D_RenderListRasterization(psurface, list, pface->transform_vertex[0], pface->transform_vertex[1], pface->transform_vertex[2], ptex,
                                                  PX_COLOR(0, 0, 0, 0), camera->zbuffer, (px_int)camera->viewport_width);
                }
            }
        }
    }
    ...
    if (list->PX_3D_PRESENTMODE & PX_3D_PRESENTMODE_LINE) {
        for (i = 0; i < list->facestream.size; i++) {
            pface = PX_VECTORAT(PX_3D_Face, &list->facestream, i);
            if (!(pface->state & PX_3D_FACESTATE_BACKFACE || pface->state & PX_3D_FACESTATE_CLIPPED)) {
                PX_GeoDrawLine(psurface, (px_int)pface->transform_vertex[0].position.x, (px_int)pface->transform_vertex[0].position.y,
                               (px_int)pface->transform_vertex[1].position.x, (px_int)pface->transform_vertex[1].position.y, 1, pface->transform_vertex[0].clr);
                PX_GeoDrawLine(psurface, (px_int)pface->transform_vertex[1].position.x, (px_int)pface->transform_vertex[1].position.y,
                               (px_int)pface->transform_vertex[2].position.x, (px_int)pface->transform_vertex[2].position.y, 1, pface->transform_vertex[1].clr);
                PX_GeoDrawLine(psurface, (px_int)pface->transform_vertex[2].position.x, (px_int)pface->transform_vertex[2].position.y,
                               (px_int)pface->transform_vertex[0].position.x, (px_int)pface->transform_vertex[0].position.y, 1, pface->transform_vertex[2].clr);
            }
        }
    }
}

px_void PX_3D_RenderListSetPixelShader(PX_3D_RenderList *list, PX_3D_PixelShader func) { list->pixelShader = func; }
...
px_void PX_3D_RenderListCullOutSide(PX_3D_RenderList *list, PX_3D_Camera *camera) {
    ...
    for (i = 0; i < list->facestream.size; i++) {
        ...
        if (pface->transform_vertex[0].position.y > camera->viewplane_height / 2 && pface->transform_vertex[1].position.y > camera->viewplane_height / 2 &&
            ...
            // left
            outside = PX_TRUE;
        } else if (pface->transform_vertex[0].position.x > camera->viewplane_width / 2 && pface->transform_vertex[1].position.x > camera->viewplane_width / 2 &&
                   pface->transform_vertex[2].position.x > camera->viewplane_width / 2) {
            // right
            outside = PX_TRUE;
        }
...
px_void PX_3D_RenderListCullRegion(PX_3D_RenderList *list, PX_3D_Camera *camera) {
    ...
    for (i = 0; i < list->facestream.size; i++) {
        ...
        if (pface->transform_vertex[0].position.z > camera->far_z && pface->transform_vertex[1].position.z > camera->far_z &&
...
px_void PX_3D_RenderListReset(PX_3D_RenderList *list) { PX_VectorClear(&list->facestream); }

px_void PX_3D_RenderListFree(PX_3D_RenderList *list) { PX_VectorFree(&list->facestream); }
...
px_void PX_3D_CameraFree(PX_3D_Camera *camera) { MP_Free(camera->mp, camera->zbuffer); }
