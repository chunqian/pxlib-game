...
px_int PX_FontModuleDrawCharacter(px_surface *psurface, PX_FontModule *mod, int x, int y, const px_dword code, px_color Color) {
    ...
    if (pChar) {
        PX_ShapeRender(psurface, &pChar->shape, x + pChar->header.BearingX, y + mod->max_BearingY - pChar->header.BearingY, PX_TEXTURERENDER_REFPOINT_LEFTTOP,
                       Color);
...
px_int PX_FontModuleDrawText(px_surface *psurface, PX_FontModule *mod, int x, int y, PX_ALIGN align, const px_char *Text, px_color Color) {
    ...
    while (PX_TRUE) {
        ...
        if (code == ' ') {
            ...
            pChar = (PX_FontModule_Charactor *)PX_MapGet(&mod->characters_map, hex);
            if (pChar) {
                PX_ShapeRender(psurface, &pChar->shape, dx + pChar->header.BearingX, dy + mod->max_BearingY - pChar->header.BearingY,
                               PX_TEXTURERENDER_REFPOINT_LEFTTOP, Color);
                dx += pChar->header.Advance;
            }
        }
    }
