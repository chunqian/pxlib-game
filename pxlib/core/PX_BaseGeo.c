...
static px_void PX_DrawLinePixel_RecK(px_surface *psurface, px_int x, px_int y, px_color color) { PX_SurfaceDrawPixel(psurface, y, x, color); }
...
px_void PX_GeoDrawCircle(px_surface *psurface, px_int x, px_int y, px_int Radius, px_int lineWidth, px_color color) {
    ...
    if (x >= 0 - Radius && y >= 0 - Radius && x < psurface->width + Radius && y < psurface->height + Radius) {
        ...
        if (i < (Radius - lineWidth / 2) * (Radius - lineWidth / 2) &&
            !PX_isPointInRect(PX_POINT((px_float)x, (px_float)y, 0), PX_RECT(0, 0, (px_float)psurface->width, (px_float)psurface->height))) {
            return;
        }
    } else {
        return;
...
px_void PX_GeoDrawSector(px_surface *psurface, px_int x, px_int y, px_int Radius_outside, px_int Radius_inside, px_color color, px_uint start_angle,
                         px_uint end_angle) {
    px_int s_quadrant, e_quadrant, trim;
    px_float start_mathRegion, end_mathRegion;
    px_int rx, ry, dy, i, xleft, xright, Sy, cY, drx, dry;
...
px_void PX_GeoDrawSolidRoundRect(px_surface *psurface, px_int left, px_int top, px_int right, px_int bottom, px_float roundRaduis, px_color color) {
    ...
    for (y = 0; y < (px_int)roundRaduis; y++) {
        ...
        for (; x < (px_int)roundRaduis; x++) {
            ...
            if (dis < roundRaduis + 0.5f) {
                ...
                if (dis > roundRaduis) drawColor._argb.a = (px_uchar)(color._argb.a * (dis - roundRaduis + 0.5f));
    ...
    if (Height > 2 * (px_int)roundRaduis) PX_GeoDrawRect(psurface, left, top + (px_int)roundRaduis, right, bottom - (px_int)roundRaduis, color);
}

px_void PX_GeoDrawRoundRect(px_surface *psurface, px_int left, px_int top, px_int right, px_int bottom, px_float roundRaduis, px_float linewidth,
                            px_color color) {
    px_int y, x, Height, Width;
    px_float r_x, r_y, dis;
    px_int drawWidth;
    ...
    for (y = 0; y < (px_int)roundRaduis; y++) {
        ...
        for (; x < (px_int)roundRaduis; x++) {
            ...
            if (dis < roundRaduis + 0.5f && dis > roundRaduis - linewidth - 0.5f) {
                ...
                if (dis < roundRaduis - linewidth + 0.5f) drawColor._argb.a = (px_uchar)(color._argb.a * (1.0f - (roundRaduis - linewidth + 0.5f - dis)));
