...
px_void PX_TuningInitialize(px_memorypool *mp, PX_Tuning *tuning, px_int SampleRate, px_double pitchShift, px_double timeShift, px_double window[],
                            px_double filter[], px_double fix[], PX_TUNING_WINDOW_SIZE windowsize) {
    px_int i;
    px_int N;
...
px_void PX_TuningSetTimeScale(PX_Tuning *tuning, px_double TimeScale) { tuning->timeScale = TimeScale; }

px_void PX_TuningSetFilter(PX_Tuning *tuning, px_double filter[]) { PX_memcpy(tuning->filter, filter, sizeof(px_double) * tuning->N); }

px_void PX_TuningSetFix(PX_Tuning *tuning, px_double fix[]) { PX_memcpy(tuning->fix, fix, sizeof(px_double) * tuning->N); }
