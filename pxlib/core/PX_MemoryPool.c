...
px_void MP_UnreleaseInfo(px_memorypool *mp) {
    ...
    for (i = 0; i < sizeof(mp->DEBUG_allocdata) / sizeof(mp->DEBUG_allocdata[0]); i++) {
        if (mp->DEBUG_allocdata[i].addr) printf("Warning:Unreleased memory in MID %p\n", mp->DEBUG_allocdata[i].addr);
    }
}
...
px_void MP_ErrorCatch(px_memorypool *Pool, PX_MP_ErrorCall ErrorCall) { Pool->ErrorCall_Ptr = ErrorCall; }
