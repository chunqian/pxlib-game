...
Game_Object_Stone *Game_Object_GetStone(PX_Object *pObject) { return (Game_Object_Stone *)pObject->pObject; }
...
px_void Game_Object_StoneUpdate(PX_Object *pObject, px_dword elpased) {
    ...
    if (pObject->x < -GAME_STONE_GEN_SPACE - 1 || pObject->x > pst->pWorld->world_width + GAME_STONE_GEN_SPACE + 1 || pObject->y < -GAME_STONE_GEN_SPACE - 1 ||
        pObject->y > pst->pWorld->world_height + GAME_STONE_GEN_SPACE + 1) {
        PX_WorldRemoveObject(pst->pWorld, pObject);
    }
}
...
px_void Game_Object_StoneRender(px_surface *psurface, PX_Object *pObject, px_dword elpased) {
    ...
    if (pst->beAttackElpased < 50) {
        ...
        blend.hdr_G = 0;
        blend.hdr_R = 1;
        PX_TextureRenderEx(psurface, pst->pTexture, (px_int)pObject->x, (px_int)pObject->y, PX_TEXTURERENDER_REFPOINT_CENTER, &blend, pst->scale,
                           pst->rotation);
    } else
        PX_TextureRenderEx(psurface, pst->pTexture, (px_int)pObject->x, (px_int)pObject->y, PX_TEXTURERENDER_REFPOINT_CENTER, PX_NULL, pst->scale,
                           pst->rotation);
...
PX_Object *Game_Object_StoneCreateObject(PX_Instance *pIns, PX_World *pWorld, px_float life, px_float velocity) {
    ...
    pst->rotation = 0;
    pst->rotationSpeed =
        (px_float)(GAME_STONE_DEFALUE_MIN_ROTATION_SPEED + (Game_rand() % (GAME_STONE_DEFALUE_MAX_ROTATION_SPEED - GAME_STONE_DEFALUE_MIN_ROTATION_SPEED)));
    pst->scale = scale;
