...
px_void Game_Object_BlackHoleExplosionRender(px_surface *psurface, PX_Object *pObject, px_dword elpased) {
    ...
    if (pDesc->soundPlayElpased += elpased) {
        if (pDesc->soundPlayElpased > 1000) {
            ...
            PX_PlayLocalSoundFromResource(pDesc->pIns, pDesc->pWorld, pObject->x + pDesc->pWorld->offsetx, pObject->y + pDesc->pWorld->offsety,
                                          GAME_TEXT_KEY_BLACKHOLE_EXPLOSIONSOUND, PX_FALSE);
        }
    }
    ...
    if (pDesc->elpased < GAME_OBJECT_BLACKHOLEEXPLOSION_STAGE1) {
        ...
        PX_GeoDrawSolidCircle(psurface, (px_int)pObject->x, (px_int)pObject->y,
                              (px_int)(eTime * 1.0f / GAME_OBJECT_BLACKHOLEEXPLOSION_STAGE1 * GAME_OBJECT_BLACKHOLEEXPLOSION_SEARCH_DAMAGEREGION),
                              PX_COLOR(255, 0, 0, 0));
        ...
        for (i = 0; i < PX_COUNTOF(pDesc->echo); i++) {
            if (pDesc->echo[i].alive) {
                ...
                PX_GeoDrawSolidCircle(psurface, (px_int)(pObject->x + pDesc->echo[i].resx * schedule), (px_int)(pObject->y + pDesc->echo[i].resy * schedule),
                                      (px_int)(pDesc->echo[i].resSize * schedule), PX_COLOR((px_uchar)(255 * (1 - schedule)), 0, 0, 0));
            }
        }

        PX_GeoDrawSolidCircle(psurface, (px_int)(pObject->x), (px_int)(pObject->y), GAME_OBJECT_BLACKHOLEEXPLOSION_SEARCH_DAMAGEREGION + (PX_rand() % 6),
                              PX_COLOR(255, 0, 0, 0));
