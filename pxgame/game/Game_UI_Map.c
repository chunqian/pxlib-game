...
px_void Game_UI_MapRender(px_surface *renderSurface, Game_UI_Map *map, px_dword elpased) {
            ...
            switch (pObject->Type) {
                ...
                case GAME_OBJECT_TYPE_MISSILE: {
                    ...
                    switch (pObject->Type) {
                        ...
                        case GAME_OBJECT_TYPE_TREASURE:
                        case GAME_OBJECT_TYPE_TREASURETAG: {
                            PX_ShapeRenderEx(&map->renderTarget, &map->map_treasure, localX, localY, PX_TEXTURERENDER_REFPOINT_CENTER, PX_COLOR(255, 255, 0, 0),
                                             1.0f, map->rotation);
                        } break;
                    }
                } break;
    ...
    PX_TextureRender(renderSurface, &map->renderTarget, renderSurface->width - map->renderTarget.width - 1,
                     renderSurface->height - map->renderTarget.height - 1, PX_TEXTURERENDER_REFPOINT_LEFTTOP, PX_NULL);
}
