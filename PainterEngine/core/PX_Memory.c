...
px_bool PX_MemoryCat(px_memory *memory, px_void *buffer, px_int size) {
    ...
    if (memory->usedsize + size > memory->allocsize) {
        ...
        if (old) PX_memcpy(memory->buffer, old, memory->usedsize);

        PX_memcpy(memory->buffer + memory->usedsize, buffer, size);

        if (old) MP_Free(memory->mp, old);
    } else {
        PX_memcpy(memory->buffer + memory->usedsize, buffer, size);
    }
...
px_bool PX_MemoryResize(px_memory *memory, px_int size) { return PX_MemoryAlloc(memory, size); }
...
px_void PX_MemoryClear(px_memory *memory) { memory->usedsize = 0; }
