...
px_bool PX_QuadtreeCreate(px_memorypool *mp, PX_Quadtree *pQuadtree, px_float mapStartX, px_float mapStartY, px_float mapWidth, px_float mapHeight,
                          px_int ObjectsCount, px_int deep) {
    pQuadtree->mp = mp;
    ...
    while (pnode) {
        ...
        if (pBox->userdata.ptr != testBox.userdata.ptr) {
            ...
            if (PX_isRectCrossRect(r1, r2)) {
                ...
                if (i == pQuadtree->Impacts.size) PX_VectorPushback(&pQuadtree->Impacts, &pBox->userdata);
            }
        }
        pnode = pnode->pnext;
    }
...
px_void PX_QuadtreeResetTest(PX_Quadtree *pQuadtree) { PX_VectorClear(&pQuadtree->Impacts); }
