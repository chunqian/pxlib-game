...
px_uint PX_BitmapGetHeight(void *BitmapBuffer) { return (px_dword)(*(px_dword *)((px_char *)BitmapBuffer + 22)); }

px_uint PX_BitmapGetWidth(void *BitmapBuffer) { return (px_dword)(*(px_dword *)((px_char *)BitmapBuffer + 18)); }

px_word PX_BitmapGetBitCount(void *BitmapBuffer) { return (px_word)(*(px_word *)((px_char *)BitmapBuffer + 28)); }

px_dword PX_BitmapGetDataOffset(void *BitmapBuffer) { return (px_dword)(*(px_dword *)((px_char *)BitmapBuffer + 10)); }
        ...
        if (Height > 0) {
            for (i = 0; i < Height; i++) {
                ...
                for (j = 0; j < Width; j++) {
                    ...
                    if (j + x < psurface->width && i + y < psurface->height) PX_SurfaceSetPixel(psurface, j + x, i + y, PXcolor32);
                }
            }
        } else {
            for (i = 0; i < -Height; i++) {
                ...
                for (j = 0; j < Width; j++) {
                    ...
                    PXcolor32._argb.g = rgb24[j].G;
                    PXcolor32._argb.b = rgb24[j].B;
                    if (j + x > 0 && i + y > 0 && j + x < psurface->width && i + y < psurface->height) PX_SurfaceSetPixel(psurface, j + x, i + y, PXcolor32);
                }
            }
        }
        return;
