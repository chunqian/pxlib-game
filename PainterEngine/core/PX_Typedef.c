...
px_double PX_exp(px_double x) { return PX_pow_dd(PX_e, x); }
...
px_float PX_tan_radian(px_float radius) { return PX_sin_radian(radius) / PX_cos_radian(radius); }
...
px_float PX_tan_angle(px_float angle) { return PX_sin_angle(angle) / PX_cos_angle(angle); }

px_float PX_Point_sin(px_point v) { return v.y / PX_sqrt(v.x * v.x + v.y * v.y); }

px_float PX_Point_cos(px_point v) { return v.x / PX_sqrt(v.x * v.x + v.y * v.y); }
...
static px_double Tail(px_double x) {
    ...
    if ((9007199254740992.0) <= x) return HalfPi + Tiny;

    y = 1 / x;
    y2 = y * y;

    return p001 -
           ((((((((((((+p23) * y2 + p21) * y2 + p19) * y2 + p17) * y2 + p15) * y2 + p13) * y2 + p11) * y2 + p09) * y2 + p07) * y2 + p05) * y2 + p03) * y2 * y +
            y) +
           p000;
}
...
static px_double atani0(px_double x) {
    ...
    return ((((((((((((+p25) * x2 + p23) * x2 + p21) * x2 + p19) * x2 + p17) * x2 + p15) * x2 + p13) * x2 + p11) * x2 + p09) * x2 + p07) * x2 + p05) * x2 +
            p03) *
               x2 * x +
           x;
}
...
px_int PX_sprintf8(px_char *_out_str, px_int str_size, const px_char fmt[], px_stringformat _1, px_stringformat _2, px_stringformat _3, px_stringformat _4,
                   px_stringformat _5, px_stringformat _6, px_stringformat _7, px_stringformat _8) {
    px_int length = 0;
...
px_int PX_sprintf7(px_char *str, px_int str_size, const px_char fmt[], px_stringformat _1, px_stringformat _2, px_stringformat _3, px_stringformat _4,
                   px_stringformat _5, px_stringformat _6, px_stringformat _7) {
    return PX_sprintf8(str, str_size, fmt, _1, _2, _3, _4, _5, _6, _7, PX_STRINGFORMAT_INT(0));
}

px_int PX_sprintf6(px_char *str, px_int str_size, const px_char fmt[], px_stringformat _1, px_stringformat _2, px_stringformat _3, px_stringformat _4,
                   px_stringformat _5, px_stringformat _6) {
    return PX_sprintf8(str, str_size, fmt, _1, _2, _3, _4, _5, _6, PX_STRINGFORMAT_INT(0), PX_STRINGFORMAT_INT(0));
}

px_int PX_sprintf5(px_char *str, px_int str_size, const px_char fmt[], px_stringformat _1, px_stringformat _2, px_stringformat _3, px_stringformat _4,
                   px_stringformat _5) {
...
px_int PX_sprintf3(px_char *str, px_int str_size, const px_char fmt[], px_stringformat _1, px_stringformat _2, px_stringformat _3) {
    return PX_sprintf8(str, str_size, fmt, _1, _2, _3, PX_STRINGFORMAT_INT(0), PX_STRINGFORMAT_INT(0), PX_STRINGFORMAT_INT(0), PX_STRINGFORMAT_INT(0),
...
px_int PX_sprintf2(px_char *str, px_int str_size, const px_char fmt[], px_stringformat _1, px_stringformat _2) {
    return PX_sprintf8(str, str_size, fmt, _1, _2, PX_STRINGFORMAT_INT(0), PX_STRINGFORMAT_INT(0), PX_STRINGFORMAT_INT(0), PX_STRINGFORMAT_INT(0),
...
px_int PX_sprintf1(px_char *str, px_int str_size, const px_char fmt[], px_stringformat _1) {
    return PX_sprintf8(str, str_size, fmt, _1, PX_STRINGFORMAT_INT(0), PX_STRINGFORMAT_INT(0), PX_STRINGFORMAT_INT(0), PX_STRINGFORMAT_INT(0),
...
px_int PX_sprintf0(px_char *str, px_int str_size, const px_char fmt[]) {
    return PX_sprintf8(str, str_size, fmt, PX_STRINGFORMAT_INT(0), PX_STRINGFORMAT_INT(0), PX_STRINGFORMAT_INT(0), PX_STRINGFORMAT_INT(0),
...
px_bool PX_ColorEqual(px_color color1, px_color color2) { return color1._argb.ucolor == color2._argb.ucolor; }
...
px_float PX_PointDot(px_point p1, px_point p2) { return p1.x * p2.x + p1.y * p2.y + p1.z * p2.z; }

px_float PX_Point4DDot(px_point4D p1, px_point4D p2) { return p1.x * p2.x + p1.y * p2.y + p1.z * p2.z; }
...
px_float PX_PointMod(px_point p) { return PX_sqrt(p.x * p.x + p.y * p.y + p.z * p.z); }

px_float PX_PointSquare(px_point p) { return (p.x * p.x + p.y * p.y + p.z * p.z); }
...
px_double PX_Covariance(px_double x[], px_double y[], px_int n) {
    ...
    sum = 0;
    for (i = 0; i < n; i++) sum += (x[i] - x_average) * (y[i] - y_average);
...
px_bool PX_memequ(const void *dst, const void *src, px_int size) {
    ...
    while (_movTs--) {
        if (*_4byteMovDst++ != *_4byteMovSrc++) return PX_FALSE;
    }
    _movTs = size & 3;
    while (_movTs--)
        if (*(pdst++) != *(psrc++)) return PX_FALSE;
    return PX_TRUE;
}
...
px_double PX_complexMod(px_complex a) { return PX_sqrtd(a.re * a.re + a.im * a.im); }
...
px_bool PX_strIsNumeric(const px_char *str) {
    ...
    if (!PX_charIsNumeric(str[0]) && !(str[0] == ('-'))) {
        if (str[0] != '.') return PX_FALSE;
    }
    for (CurrentCharIndex = 1; CurrentCharIndex < PX_strlen(str); CurrentCharIndex++) {
...
px_double PX_log10(px_double __x) { return PX_ln(__x) / 2.30258509299404568401799145468; }
...
px_uint32 PX_rand() { return ((px_uint32)(px_srand_seed = (px_srand_seed * 764261123) % (0xefffffff))) & PX_RAND_MAX; }

px_double PX_randRange(px_double min, px_double max) { return min + PX_rand() * 1.0 / PX_RAND_MAX * (max - min); }

px_uint32 PX_randEx(px_uint64 seed) { return (px_uint32)(seed = (seed * 764261123) % (0xefffffff)); }
...
px_double PX_Ceil(px_double v) { return v - (px_int)v ? (px_double)((px_int)(v + 1)) : v; }
...
px_uint32 PX_crc32(px_void *buffer, px_uint size) {
    ...
    for (i = 0; i < size; i++) crc = crc32tab[(crc ^ ((px_byte *)buffer)[i]) & 0xff] ^ (crc >> 8);
...
px_dword PX_htonl(px_dword h) { return PX_isBigEndianCPU() ? h : BigLittleSwap32(h); }

px_dword PX_ntohl(px_dword n) { return PX_isBigEndianCPU() ? n : BigLittleSwap32(n); }

px_word PX_htons(px_word h) { return PX_isBigEndianCPU() ? h : BigLittleSwap16(h); }

px_word PX_ntohs(px_word n) { return PX_isBigEndianCPU() ? n : BigLittleSwap16(n); }
...
px_float PX_PointDistance(px_point p1, px_point p2) { return PX_sqrt((p1.x - p2.x) * (p1.x - p2.x) + (p1.y - p2.y) * (p1.y - p2.y)); }
