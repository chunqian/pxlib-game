...
static BOOL CheckFormat(D3DFORMAT Format, BOOL Windowed, BOOL HAL) {
    if (FAILED(IDirect3D9_CheckDeviceType(DirectX_pDirect3D9, D3DADAPTER_DEFAULT, (HAL == TRUE) ? D3DDEVTYPE_HAL : D3DDEVTYPE_REF, Format, Format, Windowed)))
        return FALSE;
...
BOOL CALLBACK DInput_RecvJoyDevice(LPCDIDEVICEINSTANCE pDevicsInst, LPVOID pvRef) {
    ...
    if (DINPUT_CreateDevice(Win_Hwnd, DInput_JoyDevice[DInput_JoyDeviceIndex].guid, &DInput_JoyDevice[DInput_JoyDeviceIndex].pDIDevice,
                            DINPUT_IO_DEVICE_TYPE_JOY)) {
...
BOOL CALLBACK DInput_RecvMouseDevice(LPCDIDEVICEINSTANCE pDevicsInst, LPVOID pvRef) {
    ...
    if (DINPUT_CreateDevice(Win_Hwnd, DInput_MouseDevice[DInput_MouseDeviceIndex].guid, &DInput_MouseDevice[DInput_MouseDeviceIndex].pDIDevice,
                            DINPUT_IO_DEVICE_TYPE_MOUSE)) {
...
BOOL CALLBACK DInput_RecvKeyboardDevice(LPCDIDEVICEINSTANCE pDevicsInst, LPVOID pvRef) {
    ...
    if (DINPUT_CreateDevice(Win_Hwnd, DInput_KeyboardDevice[DInput_KeyboardDeviceIndex].guid, &DInput_KeyboardDevice[DInput_KeyboardDeviceIndex].pDIDevice,
                            DINPUT_IO_DEVICE_TYPE_KEYBOARD)) {
...
BOOL PX_D3DReset(HWND hWnd, int Width, int Height, BOOL bfullScreen) {
    ...
    if ((DirectX_pDirect3D9 = Direct3DCreate9(D3D_SDK_VERSION)) == NULL) return FALSE;

    // If failed then return
    if (DirectX_pDirect3D9 == NULL) return FALSE;

    if (FAILED(IDirect3D9_GetAdapterDisplayMode(DirectX_pDirect3D9, D3DADAPTER_DEFAULT, &DirectX_DisplayMode))) return FALSE;
    ...
    if (bfullScreen == FALSE) {
        if (!(DirectX_BPP = GetFormatBPP(DirectX_DisplayMode.Format))) return FALSE;
    }
    ...
    if ((DirectX_FullScreen = bfullScreen) == FALSE) {
        ...
        if (CheckFormat(DirectX_DisplayMode.Format, TRUE, TRUE) == TRUE) {
            ...
            if (CheckFormat(DirectX_DisplayMode.Format, TRUE, FALSE) == FALSE) return FALSE;
        }
    } else {
        // Full screen mode
    ...
    if (FAILED(hr = IDirect3D9_CreateDevice(DirectX_pDirect3D9, D3DADAPTER_DEFAULT, (DirectX_HAREWARE == TRUE) ? D3DDEVTYPE_HAL : D3DDEVTYPE_REF, hWnd,
                                            D3DCREATE_SOFTWARE_VERTEXPROCESSING, &DirectX_d3dpp, &DirectX_pD3DDevice))) {
        return FALSE;
    }
    ...
    if (FAILED(D3DXCreateSprite(DirectX_pD3DDevice, &DirectX_pD3DSprite))) return FALSE;
    ...
    if (FAILED(IDirect3DDevice9_SetTexture(DirectX_pD3DDevice, 0, DirectX_RenderTexture))) return FALSE;
...
BOOL PX_SystemRender(void *raw, int witdh, int height) {
    ...
    if (FAILED(IDirect3DDevice9_Clear(DirectX_pD3DDevice, 0, NULL, D3DCLEAR_TARGET, D3DXCOLOR(1, 0, 0, 1), 1.0, 0))) return FALSE;
    ...
    if (FAILED(DirectX_pD3DDevice->Present(0, 0, 0, 0))) ResetDevice(DirectX_d3dpp);
...
BOOL PX_MouseLButtonDown() { return DInput_MouseState.rgbButtons[0] & 0x80; }

BOOL PX_MouseRButtonDown() { return DInput_MouseState.rgbButtons[1] & 0x80; }

BOOL PX_MouseMButtonDown() { return DInput_MouseState.rgbButtons[2] & 0x80; }
