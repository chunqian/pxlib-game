...
px_int PX_ModbusPacketBuild(PX_ModbusPacket *packet, px_byte opcode, px_word startAddr, px_word regcount, px_byte *buffer, px_int size) {
    ...
    packet->unit = 1;
    if (size) PX_memcpy(packet->data, buffer, size);
    return size + 9;
}
...
px_int PX_ModbusRead(PX_ModbusPacket *packet, px_word startAddr, px_word regcount) { return PX_ModbusPacketBuild(packet, 0x03, startAddr, regcount, "", 0); }
