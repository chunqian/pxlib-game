...
static px_char *PX_Script_Keywords[] = {"IF",     "ELSE", "COMPARE", "WITH",  "WHILE",  "FOR",    "BREAK", "RETURN", "SET",    "FUNCTION",
                                        "EXPORT", "HOST", "INT",     "FLOAT", "STRING", "MEMORY", "_BOOT", "RETURN", "SIZEOF", "_ASM"};
...
px_void PX_ScriptParserClearStackVariables(PX_SCRIPT_Analysis *analysis) {
    ...
    for (i = 0; i < analysis->v_variableStackTable.size; i++) {
        ...
        if (pvar->bInitialized) PX_StringFree(&pvar->GlobalInitializeValue);
...
static px_bool PX_ScriptParseInclude(px_string *codes, PX_SCRIPT_LIBRARY *lib, px_char *name) {
    ...
    if (!PX_LexerSetSourcePointer(&lexer, codes->buffer)) goto _ERROR;
                ...
                if (PX_ScriptTranslatorNextToken(&lexer) == PX_LEXER_LEXEME_TYPE_CONATINER) {
                    ...
                    for (i = 0; i < lib->codeLibraries.size; i++) {
                        if (PX_strequ(lexer.CurLexeme.buffer, PX_VECTORAT(PX_SCRIPT_CODE, &lib->codeLibraries, i)->name.buffer)) {
                            ...
                            allocsize = lexerState.offset + PX_strlen(PX_VECTORAT(PX_SCRIPT_CODE, &lib->codeLibraries, i)->code.buffer) +
                                        PX_strlen(lexer.Sources + PX_LexerGetState(&lexer).offset - 1) + 1;
...
static px_bool PX_ScriptParsePretreatment(px_string *codes, PX_SCRIPT_LIBRARY *lib, px_char *name) {
    if (!PX_ScriptParseInclude(codes, lib, name)) goto _ERROR;

    if (!PX_ScriptParseDefine(codes, lib, name)) goto _ERROR;
...
static px_bool PX_ScriptParse_AST_PushToken(PX_SCRIPT_Analysis *analysis, px_vector *op, px_vector *tk, PX_SCRIPT_EXPR_STREAM atom, PX_SCRIPT_SET *_inset,
                                            PX_SCRIPT_SET **_outset) {
    px_int i;
    PX_SCRIPT_VARIABLES *pvar;
...
static px_bool PX_ScriptParseLastInstr_EQUAL(PX_SCRIPT_Analysis *analysis, px_vector *op, px_vector *tk, px_string *out) {
    ...
    switch (operandRight.operandType) {
        ...
        case PX_SCRIPT_AST_OPERAND_TYPE_FLOAT: {
            ...
            switch (operandLeft.operandType) {
                ...
                case PX_SCRIPT_AST_OPERAND_TYPE_INT: {
                    ...
                    if (operandLeft.operandType == PX_SCRIPT_AST_OPERAND_TYPE_INT) PX_StringCat(out, "INT R2\n");
        ...
        case PX_SCRIPT_AST_OPERAND_TYPE_STRING_IDX: {
            ...
            pTop->region = PX_SCRIPT_VARIABLE_REGION_POP;
            pTop->_oft = 0;
            if (operandLeft.operandType != PX_SCRIPT_AST_OPERAND_TYPE_INT && operandLeft.operandType != PX_SCRIPT_AST_OPERAND_TYPE_STRING_IDX &&
        ...
        case PX_SCRIPT_AST_OPERAND_TYPE_MEMORY_IDX: {
            ...
            pTop->region = PX_SCRIPT_VARIABLE_REGION_POP;
            pTop->_oft = 0;
            if (operandLeft.operandType != PX_SCRIPT_AST_OPERAND_TYPE_INT && operandLeft.operandType != PX_SCRIPT_AST_OPERAND_TYPE_MEMORY_IDX &&
...
static px_bool PX_ScriptParseLastInstr_INC(PX_SCRIPT_Analysis *analysis, px_vector *op, px_vector *tk, px_string *out) {
    ...
    PX_VectorPop(op);
    if (operand1.operandType != PX_SCRIPT_AST_OPERAND_TYPE_INT && operand1.operandType != PX_SCRIPT_AST_OPERAND_TYPE_INT_PTR &&
        operand1.operandType != PX_SCRIPT_AST_OPERAND_TYPE_FLOAT_PTR && operand1.operandType != PX_SCRIPT_AST_OPERAND_TYPE_STRING_PTR &&
        operand1.operandType != PX_SCRIPT_AST_OPERAND_TYPE_MEMORY_PTR && operand1.operandType != PX_SCRIPT_AST_OPERAND_TYPE_SET_PTR) {
        return PX_FALSE;
    }
...
static px_bool PX_ScriptParseExpressionStream(PX_SCRIPT_Analysis *analysis, px_vector *stream, px_int *offset, PX_SCRIPT_AST_OPERAND *pretOperand,
    ...
    while (*offset < stream->size) {
        ...
        switch (pVec[*offset].type) {
            case PX_SCRIPT_TRANSLATOR_EXPRESSION_TOKEN: {
                if (!PX_ScriptParse_AST_PushToken(analysis, op, tk, pVec[*offset], pcurrentSet, &pcurrentSet)) goto _ERROR;
            } break;
            case PX_SCRIPT_TRANSLATOR_EXPRESSION_OP_FUNCTION: {
                ...
                if (paramcount) {
                    ...
                    if (retOperand.operandType == PX_SCRIPT_AST_OPERAND_TYPE_SET || retOperand.operandType == PX_SCRIPT_AST_OPERAND_TYPE_SET_PTR ||
                    ...
                    switch (retOperand.operandType) {
                        ...
                        case PX_SCRIPT_AST_OPERAND_TYPE_SET_PTR:
                        case PX_SCRIPT_AST_OPERAND_TYPE_SET_PTR_CONST:
                            if (pfunc->parameters[i].type != PX_SCRIPT_PARSER_VAR_TYPE_SET_PTR ||
                                PX_ScriptParseGetSetByIndex(analysis, pfunc->parameters[i].setIndex) != retOperand.pSet) {
                                PX_ScriptTranslatorError(&analysis->lexer, "Parameter not matched.");
                                goto _ERROR;
                            }
                            break;
            ...
            case PX_SCRIPT_TRANSLATOR_EXPRESSION_OP_LBRACKETBEGIN: {
                ...
                if (!PX_ScriptParseExpressionStream(analysis, stream, offset, &retOperand, PX_SCRIPT_TRANSLATOR_EXPRESSION_OP_LBRACKETEND, out)) goto _ERROR;
            ...
            case PX_SCRIPT_TRANSLATOR_EXPRESSION_OP_MBRACKETBEGIN: {
                if (level > pVec[*offset].oplevel) {
                    ...
                    do {
                        if (op->size > 0) {
                            if (!PX_ScriptParseLastInstr(analysis, op, tk, out)) goto _ERROR;
                            if (op->size) plastop = PX_VECTORAT(PX_SCRIPT_AST_OPCODE, op, op->size - 1);
                        } else
                            break;
                ...
                (*offset)++;
                if (!PX_ScriptParseExpressionStream(analysis, stream, offset, &retOperand, PX_SCRIPT_TRANSLATOR_EXPRESSION_OP_MBRACKETEND, out)) goto _ERROR;
...
static px_bool PX_ScriptParseExpression(PX_SCRIPT_Analysis *analysis, px_char *expr, px_string *out, PX_SCRIPT_AST_OPERAND *retOperand) {
    ...
    if (!PX_LexerLoadSourceFromMemory(&lexer, expr)) return PX_FALSE;
        ...
        if (type == PX_LEXER_LEXEME_TYPE_TOKEN) {
            ...
            switch (lexer.Symbol) {
                ...
                case '(': {
                    ...
                    PX_StringCat(&Op.code, "(");
                    PX_VectorPushback(&stream, &Op);
                    accept_type = PX_SCRIPT_EXPRESSION_ACCEPT_TOKEN | PX_SCRIPT_EXPRESSION_ACCEPT_LBRACKET_START | PX_SCRIPT_EXPRESSION_ACCEPT_LBRACKET_END |
        ...
        if (analysis->functionInside) {
            for (i = 0; i < analysis->v_variableStackTable.size; i++) {
                if (PX_strequ(variable.Mnemonic.buffer, PX_VECTORAT(PX_SCRIPT_VARIABLES, &analysis->v_variableStackTable, i)->Mnemonic.buffer)) {
                    ...
                    PX_StringFree(&variable.Mnemonic);
                    if (variable.bInitialized) PX_StringFree(&variable.GlobalInitializeValue);
                    goto _ERROR;
                }
            }
...
px_bool PX_ScriptCompilerCompile(PX_SCRIPT_LIBRARY *lib, px_char *name, px_string *ASM, px_int LocalStackSize) {
    ...
    if (!PX_ScriptParsePretreatment(&codes, lib, name)) return PX_FALSE;
    ...
    while (PX_TRUE) {
        ...
        if ((pset = PX_ScriptParseGetSetInfo(&analysis, analysis.lexer.CurLexeme.buffer)) != PX_NULL ||
            ...
            if (type == PX_LEXER_LEXEME_TYPE_DELIMITER && analysis.lexer.Symbol == '(') {
                ...
                if (!PX_ScriptParseFunctionDefined(&analysis, PX_SCRIPT_TRANSLATOR_FUNCTION_TYPE_CUSTOM)) goto _ERROR;
        ...
        if (PX_strequ(analysis.lexer.CurLexeme.buffer, PX_SCRIPT_TRANSLATOR_KEYWORD_HOST)) {
            ...
            if (PX_strequ(analysis.lexer.CurLexeme.buffer, PX_SCRIPT_TRANSLATOR_KEYWORD_VAR_INT) ||
                ...
                PX_LexerSetState(state);
                if (!PX_ScriptParseFunctionDefined(&analysis, PX_SCRIPT_TRANSLATOR_FUNCTION_TYPE_HOST)) goto _ERROR;
            } else {
                goto _ERROR;
            }
        ...
        if (PX_strequ(analysis.lexer.CurLexeme.buffer, PX_SCRIPT_TRANSLATOR_KEYWORD_EXPORT)) {
            ...
            if (PX_strequ(analysis.lexer.CurLexeme.buffer, PX_SCRIPT_TRANSLATOR_KEYWORD_VAR_INT) ||
                ...
                PX_ScriptParseGetSetInfo(&analysis, analysis.lexer.CurLexeme.buffer) != PX_NULL) {
                PX_LexerSetState(state);
                if (!PX_ScriptParseFunctionDefined(&analysis, PX_SCRIPT_TRANSLATOR_FUNCTION_TYPE_EXPORT)) goto _ERROR;
            } else {
                goto _ERROR;
            }
    ...
    PX_StringInit(analysis.mp, &fmrString);
    PX_StringFormat3(&fmrString, ".GLOBAL %1\n.STACK %2\n.THREAD %3\n", PX_STRINGFORMAT_INT(globalSize), PX_STRINGFORMAT_INT(LocalStackSize),
...
px_char *PX_ScriptCompilerError() { return PX_Script_InterpreterError; }
