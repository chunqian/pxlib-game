...
px_bool PX_RuntimeInitialize(PX_Runtime *pe, px_int width, px_int height, px_void *runtime_memoryPtr, px_uint size, px_uint ui_size, px_uint resource_size,
                             px_uint game_size) {
    pe->width = width;
    pe->height = height;
...
px_memorypool PX_RuntimeCreateCalcMemoryPool(PX_Runtime *pe) { return MP_Create(MP_Malloc(&pe->mp, PE_MEMORY_CALC_SIZE), PE_MEMORY_CALC_SIZE); }

px_void PX_RuntimeFreeCalcMemoryPool(PX_Runtime *pe, px_memorypool mp) { MP_Free(&pe->mp, mp.StartAddr); }
