...
Game_Object_ShotGun *Game_Object_GetShotGun(PX_Object *pObject) { return (Game_Object_ShotGun *)pObject->pObject; }
...
px_void Game_Object_ShotGunUpdate(PX_Object *pObject, px_dword elpased) {
    ...
    if (pObject->x < 0 || pObject->x > psg->pWorld->world_width || pObject->y < 0 || pObject->y > psg->pWorld->world_height) {
        PX_WorldRemoveObject(psg->pWorld, pObject);
    }
}
...
PX_Object *Game_Object_ShotGunCreateObject(PX_Instance *pIns, PX_World *pWorld, px_point position, px_point direction, px_point velocity, px_dword impactTest,
                                           px_int player) {
    PX_Object *pObject;
    px_texture *pTexture;
...
px_void Game_Object_ShotgunCreate(PX_Instance *pIns, PX_World *pWorld, px_point position, px_point direction, px_point refvelocity, px_int group,
                                  px_int player) {
    PX_Object *pObject;
    px_point velocity;
    ...
    pObject =
        Game_Object_ShotGunCreateObject(pIns, pWorld, position, direction, velocity, Game_Object_GroupImpactTestType(group) | GAME_IMPACT_TYPE_AMMO, player);
