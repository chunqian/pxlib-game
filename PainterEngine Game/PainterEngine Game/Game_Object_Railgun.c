...
px_void Game_Object_RailgunUpdate(PX_Object *pObject, px_dword elpased) {
    ...
    if (pObject->x < 0 || pObject->x > pRailgun->pWorld->world_width || pObject->y < 0 || pObject->y > pRailgun->pWorld->world_height) {
        PX_WorldRemoveObject(pRailgun->pWorld, pObject);
    }
}
...
PX_Object *Game_Object_RailgunCreateObject(PX_Instance *pIns, PX_World *pWorld, px_point position, px_point direction, px_point velocity, px_dword impactTest,
                                           px_int player) {
    PX_Object *pObject;
...
px_void Game_Object_RailgunCreate(PX_Instance *pIns, PX_World *pWorld, px_point position, px_point direction, px_point refvelocity, px_int group,
    ...
    pObject = Game_Object_RailgunCreateObject(pIns, pWorld, position, direction, velocity, Game_Object_GroupImpactTestType(group) ^ (GAME_IMPACT_TYPE_NATURE),
                                              player);
    if (pObject) {
