...
Game_Object_Piercer *Game_Object_GetPiercer(PX_Object *pObject) { return (Game_Object_Piercer *)pObject->pObject; }
...
px_void Game_Object_PiercerRender(px_surface *psurface, PX_Object *pObject, px_dword elpased) {
    ...
    if (pDesc->alive < 300) {
        ...
        PX_AnimationRender_vector(psurface, &pDesc->animation, PX_POINT(pObject->x, pObject->y, 0), pDesc->direction, PX_TEXTURERENDER_REFPOINT_CENTER,
                                  PX_NULL);
    }
}

PX_Object *Game_Object_PiercerCreateObject(PX_Instance *pIns, PX_World *pWorld, px_point position, px_point direction, px_point velocity, px_dword impactTest,
                                           px_int player) {
    PX_Object *pObject;
...
px_void Game_Object_PiercerCreate(PX_Instance *pIns, PX_World *pWorld, px_point position, px_point direction, px_point refvelocity, px_int group,
    ...
    pObject =
        Game_Object_PiercerCreateObject(pIns, pWorld, position, direction, velocity, Game_Object_GroupImpactTestType(group) | GAME_IMPACT_TYPE_AMMO, player);
