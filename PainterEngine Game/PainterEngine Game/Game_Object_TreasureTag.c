...
Game_Object_TreasureTag *Game_Object_GetTreasureTag(PX_Object *pObject) { return (Game_Object_TreasureTag *)pObject->pObject; }
...
px_void Game_Object_TreasureTagRender(px_surface *psurface, PX_Object *pObject, px_dword elpased) {
    ...
    if (GAME_TREASURETAG_DEFALUE_ALIVE_TIME - pst->elpased < 500) {
        if (GAME_TREASURETAG_DEFALUE_ALIVE_TIME > pst->elpased) {
            ...
            PX_ShapeRenderEx(psurface, pst->pShape, (px_int)pObject->x, (px_int)pObject->y, PX_TEXTURERENDER_REFPOINT_CENTER, PX_COLOR(128, 192, 0, 0), scale,
                             pst->rotation);
        }

    } else
        PX_ShapeRenderEx(psurface, pst->pShape, (px_int)pObject->x, (px_int)pObject->y, PX_TEXTURERENDER_REFPOINT_CENTER, PX_COLOR(128, 192, 0, 0), 1.0f,
                         pst->rotation);
