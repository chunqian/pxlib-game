...
px_void Game_Object_FireExplosionRender(px_surface *psurface, PX_Object *pObject, px_dword elpased) {
    ...
    if (pfe->ring) {
        ...
        PX_GeoDrawCircle(psurface, (px_int)pObject->x, (px_int)pObject->y, (px_int)(PX_sqrt(fr) * pfe->range * 4), (px_int)(fr * pfe->range * 0.25f),
                         PX_COLOR((px_uchar)(255 * (1 - fr)), 255, (px_uchar)(255 * (1 - fr)), 0));
    }

    PX_ParticalLauncherRender(psurface, &pfe->launcher, PX_POINT(pObject->x, pObject->y, pObject->z));
}

PX_Object *Game_Object_FireExplosionCreateObject(PX_Instance *pIns, PX_World *pWorld, PX_ParticalLauncher_InitializeInfo init_info, px_point position,
                                                 px_float range, px_dword alive, px_bool ring) {
