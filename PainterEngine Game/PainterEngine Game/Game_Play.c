...
px_void Game_PlayReset(Game_Play *pPlay) {
    ...
    // Create World
    if (!PX_WorldInit(&pPlay->pIns->runtime.mp_game, &pPlay->world, GAME_WORLD_WIDTH, GAME_WORLD_HEIGHT, pPlay->pIns->runtime.RenderSurface.width,
                      pPlay->pIns->runtime.RenderSurface.height, GAME_PARTICAL_MP_CALCULATE))
        return;
...
px_void Game_PlaySetGameTime(Game_Play *pPlay, px_dword gameTime) { pPlay->GameTime = gameTime; }
...
px_void Game_PlayRender(px_surface *psurface, Game_Play *pPlay, px_dword elpased) { PX_WorldRender(psurface, &pPlay->world, elpased); }
...
px_int Game_PlayIsPlayerAlive(Game_Play *pPlay, px_int Player) { return pPlay->Players[Player].pShipObject && pPlay->Players[Player].pShipObject->Enabled; }
