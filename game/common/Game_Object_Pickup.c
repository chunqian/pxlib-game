...
px_void Game_Object_PickupUpdate(PX_Object *pObject, px_dword elpased) {
    ...
    if (pObject->x < 0 || pObject->x > pPickup->pWorld->world_width || pObject->y < 0 || pObject->y > pPickup->pWorld->world_height) {
        PX_WorldRemoveObject(pPickup->pWorld, pObject);
    }
}
...
PX_Object *Game_Object_PickupCreateObject(PX_Instance *pIns, PX_World *pWorld, px_point position, GAME_OBJECT_PICKETUP_TYPE type, px_dword impactTest,
