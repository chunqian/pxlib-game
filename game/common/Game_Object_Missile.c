...
px_void Game_Object_MissileUpdate(PX_Object *pObject, px_dword elpased) {
    ...
    if (pObject->x < 0 || pObject->x > pMissile->pWorld->world_width || pObject->y < 0 || pObject->y > pMissile->pWorld->world_height) {
        PX_WorldRemoveObject(pMissile->pWorld, pObject);
    }
}
...
px_void Game_Object_MissileRender(px_surface *psurface, PX_Object *pObject, px_dword elpased) {
    ...
    if (pMissile->elpased >= GAME_OBJECT_MISSILE_PREPARE_TIME) {
        if (!pMissile->brun) {
            ...
            PX_PlayLocalSoundFromResource(pMissile->pIns, pMissile->pWorld, pObject->x + pMissile->pWorld->offsetx, pObject->y + pMissile->pWorld->offsety,
                                          GAME_TEXT_KEY_MISSILE_LAUNCH, PX_FALSE);
        }
    }
    ...
    if (alive < 300) {
        ...
        PX_AnimationRender_vector(psurface, &pMissile->animation, PX_POINT(pObject->x, pObject->y, 0), pMissile->direction, PX_TEXTURERENDER_REFPOINT_CENTER,
                                  &blend);
    } else {
        PX_AnimationRender_vector(psurface, &pMissile->animation, PX_POINT(pObject->x, pObject->y, 0), pMissile->direction, PX_TEXTURERENDER_REFPOINT_CENTER,
                                  PX_NULL);
