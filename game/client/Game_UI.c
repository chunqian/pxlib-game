...
px_bool Game_UIInitialize(PX_Instance *pIns, Game_UI *ui, Game_Play *pPlay) {
    ...
    if (!Game_UI_StatusPanelInitialize(pIns, &ui->uiPanel, pPlay)) return PX_FALSE;

    if (!Game_UI_MapInitialize(pIns, &ui->uiMap, pPlay, 256)) return PX_FALSE;

    if (!Game_UI_ScorePanelInitialize(&ui->uiScorePanel, pIns, pPlay)) return PX_FALSE;

    if (!Game_UI_RankPanelInitialize(&ui->uiRankPanel, pIns, pPlay)) return PX_FALSE;

    if (!Game_UI_MessageInitialize(&ui->uiMessage, pIns, pPlay)) return PX_FALSE;
...
px_void Game_UIRender(px_surface *renderSurface, Game_UI *ui, px_int monitorPlayer, px_int fps, px_dword elpased) {
    ...
    PX_TextureRenderRotation(renderSurface, ui->cursor.pTexture, (px_int)ui->pIns->Controller.Cursor.x, (px_int)ui->pIns->Controller.Cursor.y,
                             PX_TEXTURERENDER_REFPOINT_CENTER, PX_NULL, (px_int)ui->cursor.rotation);
