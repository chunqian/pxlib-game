    ...
    if (pShipObject->Enabled) {
        ...
        if (ui->weaponSwitchElpased < 2000) {
            ...
            if (pShip->WeaponStack[wi].type != GAME_OBJECT_SHIP_WEAPON_TYPE_NONE) {
                ...
                PX_ShapeRender(renderSurface, ui->weaponlogo[ri], x, y, PX_TEXTURERENDER_REFPOINT_LEFTTOP,
                               PX_COLOR(255, (px_uchar)((1 - ui->weaponSwitchElpased / 2000.f) * 255), 0, 0));
            }
            wi++;
            ...
            for (i = 0; i < PX_COUNTOF(pShip->WeaponStack) - 1; i++) {
                ...
                if (pShip->WeaponStack[wi].type != GAME_OBJECT_SHIP_WEAPON_TYPE_NONE) {
                    ...
                    PX_ShapeRender(renderSurface, ui->weaponlogo[ri], x, y, PX_TEXTURERENDER_REFPOINT_LEFTTOP,
                                   PX_COLOR((px_uchar)((1 - ui->weaponSwitchElpased / 2000.f) * 255), 0, 0, 0));
                }
                wi++;
            }
        } else {
