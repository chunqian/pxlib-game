                        ...
                        if (PX_memequ(&pR->connections[i].Addr, &Addr, sizeof(Addr))) {
                            ...
                            if (preg->active_count) {
                                if (PX_StartupServer_Database_AccountIsRegisted(pR->link_database, user)) {
                                    ...
                                    if (PX_StartupServer_Database_RegistryAccount(pR->link_database, user, nickname, pwd, regcode, preg->authority_level,
                                                                                  preg->sign_admin)) {
                                        PX_SignUp_Packet_SignAck ack;
                                        ack.magic_numeric = PX_NETWORKSIGNUP_MAGIC;
                                        ack.opcode = PX_NETWORKSIGNUP_OPCODE_SIGNACK;
                                        ...
                                        if (preg->authority_level > PX_STARTUP_SIGNUP_IP_LEVEL) PX_StartupServer_SignUpAddRegIP(pR, Addr.ipv4);
...
px_bool PX_StartupServer_SignAddRegCode(PX_StartupServer_SignUp *pSp, px_char regCode[32], px_char signAdmin[16], px_dword activeCount,
