...
px_bool PX_Application_LobbyLogin(px_byte aes_code[32], px_char account[32], px_char pwd[32], _OUT px_byte cookie[32], _OUT px_dword *map_index,
                                  _OUT px_dword *id) {
    return PX_StartupServer_LobbyLogin(&App.lobby, aes_code, account, pwd, cookie, map_index, id);
}

px_void PX_Application_ConsolePrint(const px_char *Text) { PX_ConsolePrintText(&App.Instance.console, Text); }
...
px_bool PX_Application_ConsoleVM_AddSignCode(PX_ScriptVM_Instance *Ins) {
    ...
    if (PX_StartupServer_SignAddRegCode(&App.signup, PX_ScriptVM_STACK(Ins, 0)._string.buffer, PX_ScriptVM_STACK(Ins, 1)._string.buffer,
                                        PX_ScriptVM_STACK(Ins, 2)._int, PX_ScriptVM_STACK(Ins, 3)._int)) {
