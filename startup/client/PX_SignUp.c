...
px_void PX_SignUpRegistryButtonOnLogin(PX_Object *pObject, PX_Object_Event e, px_void *user_ptr) { PX_ApplicationSwitchLogin("", ""); }

px_void PX_SignUpRegistryButtonOnExit(PX_Object *pObject, PX_Object_Event e, px_void *user_ptr) { exit(0); }
...
static px_void PX_SignUpUpdate_Failed(PX_SignUp *pR, px_dword elpased) { PX_SignUpReset(pR); }
...
px_void PX_SignUpRender(PX_SignUp *pl, px_dword elpased) {
    switch (pl->status) {
        ...
        case PX_SIGNUP_STATUS_REGISTING_REG:
        case PX_SIGNUP_STATUS_FAILED:
            PX_TextureRender(&pl->pIns->runtime.RenderSurface, &pl->SignUpTexture, pl->pIns->runtime.width / 2, pl->pIns->runtime.height / 2 - 75,
