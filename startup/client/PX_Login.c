...
px_void GameStartup_LoginButtonOnExit(PX_Object *pObject, PX_Object_Event e, px_void *user_ptr) { exit(0); }
...
px_void PX_LoginRender(PX_Login *pl, px_dword elpased) {
    switch (pl->status) {
        case PX_LOGIN_STATUS_STANDBY:
            PX_TextureRender(&pl->pIns->runtime.RenderSurface, &pl->LoginTexture, pl->pIns->runtime.width / 2, pl->pIns->runtime.height / 2 - 75,
...
px_void PX_LoginPostEvent(PX_Login *pl, PX_Object_Event e) { PX_ObjectPostEvent(pl->login_root, e); }
